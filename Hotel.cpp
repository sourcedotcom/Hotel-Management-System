...
void HOTEL::rooms()
{
    ...
    ifstream fin("Record.DAT", ios::in | ios::binary);
    vector<HOTEL> records; // Vector to store all hotel records
    ...
    // Sort the records based on room number
    for (size_t i = 0; i < records.size(); ++i)
    {
        for (size_t j = i + 1; j < records.size(); ++j)
        {
            if (records[i].room_no > records[j].room_no)
            {
                HOTEL temp = records[i];
                records[i] = records[j];
                records[j] = temp;
            }
        }
    ...
    for (size_t i = 0; i < records.size(); ++i)
    {
        cout << "\n |" << setw(NoWidth) << setfill(separator) << records[i].room_no << " |";
        cout << setw(GuestWidth) << setfill(separator) << records[i].name << " |";
        cout << setw(AddressWidth) << setfill(separator) << records[i].address << " |";
        cout << setw(RoomWidth) << setfill(separator) << records[i].rtype << " |";
        cout << setw(ContactNoWidth) << setfill(separator) << records[i].phone << " |";
    }
    cout << "\n +---------+------------------+-----------------+--------------+--------------+";
    cout << "\n\n\n\t\t\tPress any key to continue.";
    getchar();
